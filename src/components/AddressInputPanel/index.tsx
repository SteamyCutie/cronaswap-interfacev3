...
const AddressInputPanel: FC<AddressInputPanelProps> = ({ id, value, onChange }) => {
  ...
  return (
    <div
      className={`flex flex-row bg-white dark:bg-gray-800 text-gray-800 dark:text-white rounded-xl items-center transition-all text-sm my-3 border border-red ${error ? 'border-opacity-50' : 'border-opacity-0 '
        }`}
      id={id}
    >
      ...
      <div className="flex w-full h-full m-0.5 text-sm sm:w-3/5">
        <Input.Address
          ...
          value={value}
          fontSize="14px"
          className="flex w-full h-full p-3 font-bold transition-all rounded-r bg-gray-100 dark:bg-gray-800 overflow-ellipsis recipient-address-input placeholder-low-emphesis"
        />
      </div>
    </div>
  )
