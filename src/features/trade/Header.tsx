...
const getQuery = (input, output) => {
  ...
  if (input && !output) {
    return { inputCurrency: input.address || 'CRONA' }
...
const ExchangeHeader: FC<ExchangeHeaderProps> = ({ input, output, allowedSlippage, showNavs = true }) => {
  ...
  const navLinkStyle =
    'font-extrabold rounded-lg bg-blue-special text-white hover:text-white/80 dark:text-white dark:hover:text-white/80 transition-all'
  const navTextStyle =
  ...
  return (
    <div className="flex items-center justify-between mb-4 space-x-3">
      {/* {showNavs && (
        <div className="grid grid-cols-2 p-2 text-gray-800 transition-all bg-white rounded-2xl dark:bg-gray-800 hover:text-gray-800/80 dark:text-white dark:hover:text-white/80 h-14">
          <NavLink
      ...
      <div className="ml-2 text-xl font-bold transition-all text text-gray-850 dark:text-white">Trade</div>
